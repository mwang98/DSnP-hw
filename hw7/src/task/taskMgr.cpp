...
TaskMgr::add(size_t nMachines){
	...
	for(size_t i = 0 ; i < nMachines ; ++i){
		TaskNode newNode;
      // If newNode has already existed, create a new node until it isn't in the _taskHash.
		while(_taskHash.check(newNode)) newNode = TaskNode();
...
TaskMgr::assign(size_t l)
{
   ...
   TaskNode min = _taskHeap.min(); _taskHeap.delMin();
   min += l;
