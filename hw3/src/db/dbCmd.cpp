...
CmdExecStatus DBAppendCmd::exec(const string& option){
    ...
    if(!isValidVarName(key)) errorOption(CMD_OPT_ILLEGAL,key);
    else if(!myStr2Int(val,value)) errorOption(CMD_OPT_ILLEGAL,val);
    else if(dbjson.getKeySetCnt(key) != 0) cerr << "Error: Element with key \"" << key << "\" already exists!!\n";
    else dbjson.add(DBJsonElem(key,value));
...
DBPrintCmd::exec(const string& option){  
    ...
    if(!dbjson.getFlag()) cerr << "Error: DB is not created yet!!\n";
    // DB has been created
    else{
        string token;
        ...
        if(token.empty()) cout << dbjson;
        else if(!isValidVarName(option)) errorOption(CMD_OPT_ILLEGAL,option);
        else{
            bool prt = false;
            ...
            if(!prt) cerr << "Error: No JSON element with key \"" << token <<"\" is found.\n";
        }
    }
