...
public:
   ...
   bool operator == (const DBJsonElem& elm) const{ return _key == elm._key; }
...
public:
   ...
   int sum() const;
   bool getFlag() const{ return _readFlag; }
   size_t getKeySetCnt(const string& str) const{ return _keySet.count(str); }
   ...
   bool operator !() { return !_readFlag; }
   // return this if JSON file has been read in; return NLL if not.
   operator void* () const {
     if(_readFlag) return (void*)this;
     return NULL; 
   }
