...
void CirGate::buildFanIn(vector<CirGate*>& list, vector<unsigned>& floatList){
	for(size_t i = 0, n = _ip.size() ; i < n ; ++i){
		CirGate* tmp = list[_ip[i]/2];
		if(tmp == 0 or tmp -> getTypeStr() == "UNDEF"){
			if(tmp == 0){
				tmp = new FloGate(_ip[i]/2);
				list[_ip[i]/2] = tmp;
			}
			// Only record an ID once
			if(find(floatList.begin(), floatList.end(), _id) == floatList.end())
				floatList.push_back(_id);
		}
...
void CirGate::reportGate() const{
	...
	cout << "==================================================" << endl;
	cout << left << setw(49) << out; cout << "=" << endl;
	cout << "==================================================" << endl;
}
...
void CirGate::DFS(vector<unsigned>& DFSList) const{
	if(_ref == _globalRef) return;

	for(size_t i = 0, n = _fanIn.size() ; i < n ; ++i)
		_fanIn[i].gatePtr() -> DFS(DFSList);
